...
export default class Lib extends React.Component {
  ...
  handleClick(r) {
    ...
    fetch(url).then((response) => {
      ...
      feed.then((data) => {
        this.setState({ loading: false, movies: data, movie: null })
      }).catch((ex) => {
    ...
    return (
      <div style={{backgroundColor:'#fafafa'}}>
        ...
        <div style={{ display: 'flex', direction: 'column', justifyContent: 'stretch', alignItems: 'stretch' }}>
          ...
          {this.state.movies && <CardList onRowClick={this.handleDetailsClick} rowStyleMap={this.rowStyleMap} style={{ height: "85vh" }} rows={this.state.movies} cols={[{ name: "Name", display: "الاسم" }, { name: "AssetType", display: "النوع", styleMap: this.styleMap }]}></CardList>}
          {this.state.movie && <Form   {...this.state.movie}></Form>}
        </div>
      </div>
    );
  }
